...
#include <omp.h>
#define LOGD(...) \
	__android_log_print(ANDROID_LOG_INFO, "DETECT", __VA_ARGS__)
//#define LOGD(...) \
//printf(__VA_ARGS__)
...
void npddetect::draw(const signed char I[], const int w, const int h, const char* saveName)
{
	int xx = (int) ceil(w / 16) * 16;
	cv::Mat ima(h, w, CV_8UC1, (unsigned char*)I, xx);
	for(int i = 0; i < m_Xs.size(); i++)
	{
		cv::rectangle(ima, cv::Rect(m_Xs[i], m_Ys[i], m_Ss[i], m_Ss[i]), 
				cv::Scalar(128,128,128));
	}
	cv::imwrite(saveName, ima);
	return ;
}
...
/*
int npddetect::detect(signed char I[], int width, int height, int rotate_degree)
{
	reset();

	int xx = (int) ceil(width / 16) * 16;
	cv::Mat ima(height, width, CV_8UC1, (unsigned char*)I, xx);

	// rotate the matrix

	// Scan with model.
	double t = (double) cvGetTickCount();
	m_numScan = scan(ima.data, ima.cols, ima.rows);
	t = ((double)cvGetTickCount() - t) / ((double)cvGetTickFrequency()*1000.) ;
	LOGD("Time to scan the picture: %lf\n",t);
	...
	m_numDetect = filter();
	t = ((double)cvGetTickCount() - t) / ((double)cvGetTickFrequency()*1000.) ;
	LOGD("Time to merge rects: %lf\n",t);

	return m_numDetect;
}*/

int npddetect::detect(signed char I[], int width, int height)
{
	reset();
...
void npddetect::draw(const char* imgPath)
{
	//cout<<"draw(), you motherfucker!"<<endl;
	cv::Mat img = cv::imread(imgPath, 0);
	for(int i = 0; i < m_Xs.size(); i++)
	{
		cv::rectangle(img, cv::Rect(m_Xs[i], m_Ys[i], m_Ss[i], m_Ss[i]), 
				cv::Scalar(128,128,128));
	}
	cv::imwrite("1.jpg", img);
	return ;
}
int npddetect::detect(cv::Mat& ima)  //
{
	return detect((signed char*)ima.data, ima.cols, ima.rows);
	return 0;
}

int npddetect::detect(const char* imgPath)  //
{
	cout<<"detect(imgPath)!"<<endl;
	cv::Mat img = cv::imread(imgPath, 0);
	int n = detect(img);
	//cout<<"draw(), you motherfucker!"<<endl;
	for(int i = 0; i < m_Xs.size(); i++)
	...
	cout<<m_Xs.size()<<" faces detected"<<endl;
	cv::imwrite("1.jpg", img);
	//draw(imgPath);
	return n;
}
...
int npddetect::prescandetect(signed char I[], int width, int height, double stepR, double thresR)
{
	// Clear former data.
	reset();
	...
	// Scan with model.
	m_numScan = gridScan((unsigned char*)I, width, height, stepR, thresR);
...
std::vector<int>& npddetect::getXs() { return m_Xs;}
std::vector<int>& npddetect::getYs() { return m_Ys;}
std::vector<int>& npddetect::getSs() { return m_Ss;}
std::vector< float >& npddetect::getScores() { return m_Scores;}
